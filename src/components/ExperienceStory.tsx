  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--bg)' }}>
      <div className="max-w-3xl mx-auto px-4 py-16">
        <button
          onClick={() => navigate('/')}
          className="flex items-center gap-2 text-[var(--text-muted)] hover:text-[var(--text-primary)] transition-colors mb-12 group"
        >
          <svg
            className="w-4 h-4 transform -translate-x-1 group-hover:-translate-x-2 transition-transform"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
            strokeWidth={2}
        ...
        <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-2xl p-6 md:p-10">
          <div className="flex items-center gap-4 mb-8">
            ...
            <div>
              <span className="text-xs text-primary-400 font-mono">{experience.period}</span>
              <h1 className="text-2xl md:text-3xl font-display font-bold text-[var(--text-primary)] mt-1">
                {experience.role}
              </h1>
              <p className="text-accent-400 font-medium">{experience.company}</p>
              <p className="text-[var(--text-muted)] text-sm mt-0.5">{experience.location}</p>
            </div>
          </div>

          <div className="space-y-5 mb-10">
            {experience.story.split('\n\n').map((paragraph, idx) => (
              <p key={idx} className="text-[var(--text-muted)] leading-relaxed text-[15px]">
                {paragraph}
              </p>
            ))}
          </div>
          ...
          <div>
            ...
            <ul className="space-y-3">
              {experience.points.map((point, idx) => (
                <li key={idx} className="text-[var(--text-muted)] text-sm leading-relaxed flex gap-3">
