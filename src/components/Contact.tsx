  ...
  return (
    <section ref={sectionRef} id="contact" className="py-24 px-4 relative">
        ...
        <div ref={iconsRef} className="flex justify-center gap-4 flex-wrap">
          {(links ?? []).map((link) => (
            <a
              ...
              title={link.label}
              className={`group social-icon w-12 h-12 rounded-full bg-[var(--bg-card)] border border-[var(--border)] flex items-center justify-center text-[var(--text-muted)] ${getHoverClass(link.platform)} ${link.platform === 'Email' ? 'group/gmail' : ''} hover:border-[var(--border-hover)] hover:scale-110 transition-all duration-300`}
            >
      ...
      <div className="mt-16 text-center text-[var(--text-muted)] text-sm">
        ...
        <p className="mt-2">&copy; {new Date().getFullYear()} {profile?.name}.</p>
      </div>
    </section>
  )
}
