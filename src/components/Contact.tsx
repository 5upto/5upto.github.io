...
const socialLinks = [
  ...
  // {
  //   label: 'WhatsApp',
  //   href: 'https://wa.me/8801721655260',
  //   icon: (
  ...
  {
    label: 'LinkedIn',
    href: 'https://linkedin.com/in/5upto',
    icon: (
      <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
  ...
  },
  {
    ...
    href: 'https://scholar.google.com/citations?user=IquMx3kAAAAJ&hl=en',
    icon: (
      <span className="relative w-5 h-5">
        ...
        <svg className="absolute inset-0 w-5 h-5 opacity-0 group-hover:opacity-100 transition-opacity duration-300" viewBox="0 0 122.88 122.88">
  ...
  {
    label: 'Instagram',
    href: 'https://instagram.com/5upto',
    icon: (
      <svg className="w-5 h-5 insta-svg" viewBox="0 0 24 24">
        <defs>
  ...
  },
  {
    ...
    icon: (
      <span className="relative w-5 h-5">
        ...
        <svg className="absolute inset-0 w-5 h-5 opacity-0 group-hover:opacity-100 transition-opacity duration-300" viewBox="0 0 24 24">
...
export default function Contact() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      const items = iconsRef.current?.querySelectorAll('.social-icon')
      if (items) {
        gsap.fromTo(items,
          { y: 20, opacity: 0, scale: 0.8 },
          { y: 0, opacity: 1, scale: 1, duration: 0.4, stagger: 0.08, ease: 'back.out(1.7)', scrollTrigger: { trigger: sectionRef.current, start: 'top 70%' } }
        )
      }
    }, sectionRef)
    return () => ctx.revert()
  ...
  return (
    <section ref={sectionRef} id="contact" className="py-24 px-4 relative">
      <div className="max-w-3xl mx-auto text-center">
        ...
        <div ref={iconsRef} className="flex justify-center gap-4 flex-wrap">
          {socialLinks.map((link) => (
            <a
              ...
              title={link.label}
              className={`group social-icon w-12 h-12 rounded-full bg-[var(--bg-card)] border border-[var(--border)] flex items-center justify-center text-[var(--text-muted)] ${link.hover} ${link.label === 'Email' ? 'group/gmail' : ''} hover:border-[var(--border-hover)] hover:scale-110 transition-all duration-300`}
            >
              {link.icon}
            </a>
          ))}
      ...
      <div className="mt-16 text-center text-[var(--text-muted)] text-sm">
        ...
        <p className="mt-2">&copy; {new Date().getFullYear()} Shawon Ghosh.</p>
      </div>
    </section>
  )
}
